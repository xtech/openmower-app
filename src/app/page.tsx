...
export default function Dashboard() {
  ...
  return (
    <Page>
      ...
      >
        <HeaderStat icon={<TrendingIcon />} value={mockMowers.length} label="Active Mowers" />
        <HeaderStat
          icon={<SpeedIcon />}
          value={mockMowers.filter((m) => m.status === 'active').length}
          label="Currently Mowing"
        />
        <HeaderStat
          icon={<CheckIcon />}
          value={`${Math.round(mockMowers.reduce((acc, m) => acc + m.efficiency, 0) / mockMowers.length)}%`}
            ...
            >
              <CardContent sx={{p: 4}}>
                ...
                <Box sx={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 2, mb: 4}}>
                  <Box sx={{textAlign: 'center', p: 2, bgcolor: theme.palette.primary.light + '10', borderRadius: 2}}>
